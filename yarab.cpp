...
int main() {
    while(true){
        string input;
        cout << "\nEnter expression (surround the operation by spaces) (or type 'exit'): ";
        getline(cin, input);
        for (char& c : input)
            c = tolower(c);

        // Modified regex pattern to match both fractions and whole numbers, and optionally a '+' sign before them
